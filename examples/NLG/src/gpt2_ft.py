...
parser.add_argument('--lora_particles', type=int, default=8, help='number of lora particles')
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        #) 
        _lm_logits, _ = model(
            _input, lm_labels=None, lm_mask=_msk, label_smooth=args.label_smooth
        ) 

        # Compute loss
        if args.label_smooth > 0.0001:
            logprobs = torch.nn.functional.log_softmax(_lm_logits.view(-1, _lm_logits.size(-1)), dim=-1).view(args.lora_particles, _initial_batch, _len, _lm_logits.size(-1))
            w_alpha =  torch.nn.functional.softmax(args.renyi_alpha * logprobs, dim = 0).detach()
            logprobs = (w_alpha * logprobs).view(-1, _lm_logits.size(-1))
            nll_loss = -logprobs.gather(dim=-1, index=_target.view(-1).unsqueeze(1))
            nll_loss = nll_loss.squeeze(1)
            smooth_loss = -logprobs.mean(dim=-1)
            loss = (1.0 - args.label_smooth) * nll_loss + args.label_smooth * smooth_loss
            loss = loss.view(args.lora_particles, _initial_batch, _len)
        else:
            loss_fct = nn.CrossEntropyLoss(ignore_index=-1, reduce=False)
            loss = loss_fct(_lm_logits.view(-1, _lm_logits.size(-1)), _target.view(-1)).view(args.lora_particles, _initial_batch, _len)
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.sm':
        config = GPT2Config(
            ...
            lora_particles = args.lora_particles,
        )
