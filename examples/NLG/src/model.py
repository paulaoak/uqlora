...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        self.scale = scale
        self.c_attn = lora.BatchedMergedLinear(
            base_linear = nn.Linear(nx, n_state * 3, bias = False), 
            M = config.lora_particles,
    ...
    def _attn(self, q, k, v, len_kv=None):
        ...
        # q : (M * batch, head, q_seq_length, head_features)
        # k : (M * batch, head, head_features, kv_seq_length)
        # w : (M * batch, head, q_seq_length, kv_seq_length)
        # v : (M * batch, head, kv_seq_length, head_features)
    ...
    def forward(self, x, history=None, layer_past=None, len_past=None):
        ...
        if layer_past is not None:
            # key : (M * batch, head, head_features, seq_length)
            # value : (M * batch, head, seq_length, head_features)
            # layer_past, key : (M * batch, head, seq_length, head_features)
            if len_past is None:
